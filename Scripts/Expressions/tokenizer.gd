...
static func tokenize(text : String) -> Array[Token]:
	...
	while i < text.length():
		...
		# handling functions
		if found.length() >= 2 and _find_function(current_char):
			# if the last n characters are a function name
			if found.ends_with(function_found):
				# remove the previous characters of the function
				found = found.substr(0, found.length() - function_found.length()) 
		...
		# 1. eg.: ... 12i+... or ... 2sin(...)...
		# 2. eg.: ... 1/2 e...
		# length < tokens.size() means that we have added something earlier (like an function or operation)
		if (length < tokens.size() or current_char == " ") and found.length() > 0:
			tokens.insert(tokens.size() - 1, Token.new(Token.TokenType.VALUE, found))
			found = ""
	...
	if found.length() > 0:
		tokens.append(Token.new(Token.TokenType.VALUE, found))
...
static func insert_multiplication(tokens : Array[Token]) -> Array[Token]:
	...
	while i < tokens.size() - 1:
		...
		if (expected_next != null) and (tokens[i+1].type in expected_next):
			tokens.insert(i+1, Token.new(Token.TokenType.OPERATOR, '*'))
